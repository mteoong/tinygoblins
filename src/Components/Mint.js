         ...
         contract.methods.numberMinted(account).call().then(function(result) {
            if (Number(result) < 1) {
               if (amount == 1) {
                  ...
                  contract.methods.mint(amount).send({ from: account, value : ((amount - 1) * 6900000000000000).toString()});
               }
            } else {
               contract.methods.mint(amount).send({ from: account, value : ((amount - 1) * 6900000000000000).toString()});
            }
