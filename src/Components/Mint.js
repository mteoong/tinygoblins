...
const checkStatus = async (mintButton, amount) => {
   ...
   if (accounts.length == 0){
      ...
      mintButton.onclick = () => {
         contract.methods.numberMinted(account).call().then(function(result) {
            if (Number(result) < 1) {
               if (amount == 1) {
                  contract.methods.mint(amount).send({ from: account, value : '0'});
               } else {
                  contract.methods.mint(amount).send({ from: account, value : ((amount) * 6900000000000000).toString()});
               }
            } else {
               contract.methods.mint(amount).send({ from: account, value : ((amount) * 6900000000000000).toString()});
            }
